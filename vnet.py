...
class ResNet32(MetaModule):
    def __init__(self, num_classes, block=BasicBlock, num_blocks=[5, 5, 5], in_channels = 3):
        super(ResNet32, self).__init__()
        self.in_planes = 16

        self.conv1 = MetaConv2d(in_channels, 16, kernel_size=3, stride=1, padding=1, bias=False)
...
def unit_tests_vnet():
    ...
    inputs = Variable(torch.randn(100, 1)).to('cuda')
    target = Variable(torch.FloatTensor(100).uniform_(0, 2).long()).to('cuda')

    optimizer_c = torch.optim.SGD(model.params(), 1e-3,
    ...
    prev = model.linear1.weight.clone()
